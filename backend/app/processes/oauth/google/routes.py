...
from backend.app.processes.auth.routes import (
    _audit_from,
    _client_ip,
    _set_auth_cookies,
)
...
from backend.app.processes.oauth.google.types import SignInOutcome
...
async def start(
    ...
    try:
        auth_url = await service.begin_signin(_audit_from(request))
...
async def callback(
    ...
    try:
        outcome = await service.complete_callback(
            code=code, state=state, audit=_audit_from(request)
        )
    ...
    if isinstance(outcome, SignInOutcome):
        ...
        _set_auth_cookies(response=response, session=outcome.session, request=request)
        return response
