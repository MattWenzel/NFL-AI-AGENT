...
class ExportService:
    ...
    async def list_exports(self, user_id: int) -> list[ExportRecord]:
        return await self.store.list_exports(user_id=user_id)

    async def get_export_detail(
        self,
        export_id: str,
        user_id: int,
    ) -> tuple[ExportRecord, list[dict], bool]:
        ...
        return record, preview_rows, preview_truncated

    async def rename_export(self, export_id: str, title: str, user_id: int) -> ExportRecord:
