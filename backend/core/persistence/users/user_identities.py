"""Per-user auth identities (password, Google OAuth, future providers).

Mixed into `RuntimeStore`. One `users` row can have multiple identities —
e.g. a password-created user who later links Google will have two rows in
`user_identities`. Deleting the user cascades via FK.

The `(provider, provider_subject)` pair is globally unique: no two users
can share a Google `sub`, and no two users can share the password sentinel
for a given email.
"""
...
from backend.core.persistence.errors import IdentityConflictError
from backend.core.persistence.models import UserIdentityRecord, UserRecord, new_id, utcnow
