"""Email verification tokens for signup (and future password reset).

Mixed into `RuntimeStore`. Tokens are one-shot: `consume_verification` atomically
verifies the token is unexpired and unused, marks it used, and returns the
user_id. The used_at column retains consumed rows for audit visibility rather
than deleting — call `purge_expired` periodically to trim.
"""
...
from backend.core.persistence.models import EmailVerificationRecord, utcnow
