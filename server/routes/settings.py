...
Keys are stored encrypted (Fernet / AES-128) via `auth.encryption`. Plaintext
...
`server/routes/codex_oauth.py` instead of accepting a pasted key. The GET
...
from storage import RuntimeStore, UserApiKeyRecord
from provider import ProviderInfo, get_provider, list_providers
