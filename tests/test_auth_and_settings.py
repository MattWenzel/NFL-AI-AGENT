...
from backend.credentials.primitives import generate_token, hash_password, verify_password
...
class TestStoreCRUD:
    ...
    async def test_touch_auth_session_updates_stale_session(self, store):
        ...
        from backend.storage.models import AuthSessionRecord
...
class TestIDOR:
    ...
    async def test_cross_user_rename_csv_returns_404(self, full_client, store):
        from backend.storage import ExportRecord  # noqa: F401
        ...
        from sqlalchemy import update as sa_update
        from backend.storage.models import ExportRecord as _ExportRecord
        async with store._async_session() as session:
            await session.execute(
...
class TestOrphanRows:
    ...
    async def test_count_orphan_rows_detects_nulls(self, store):
        ...
        from backend.storage.models import SessionRecord as _SessionRecord
