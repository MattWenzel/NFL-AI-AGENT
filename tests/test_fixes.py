...
# ---------------------------------------------------------------------------
from backend.core.tools.sandbox import _ensure_limit
...
class TestValidateSQLEdgeCases:
    ...
    def test_leading_block_comment_accepted(self):
        from backend.core.tools.sandbox import validate_sql
    ...
    def test_multi_statement_rejected_by_sqlite(self):
        ...
        from backend.core.tools.errors import SQLValidationError
...
class TestSandboxIntegration:
    ...
    def test_search_players_tool_works(self):
        ...
        _skip_if_stats_db_locked()
        from backend.core.tools.handlers.player_lookup import _search_players
...
class TestClampLimitParam:
    ...
    def test_clamps_above_max(self):
        from backend.core.tools.sandbox import _clamp_limit_param
...
# ---------------------------------------------------------------------------
from backend.core.providers.clients.anthropic import AnthropicClient
from backend.core.providers.types import Message, ToolUseEvent
...
class TestSSEDisconnectDetection:
    ...
    def test_disconnect_check_in_source(self):
        ...
        import inspect
        from backend.app.processes.chat.routes import chat_stream
...
from backend.core.agent.events import ToolCompletedEvent, ToolPendingEvent
...
class TestRuntimeTranscript:
    ...
    async def test_build_model_messages_preserves_tool_result_order(self, tmp_path):
        ...
        from backend.core.agent.message_builder import build_model_messages
...
class TestCsvExportRegistration:
    ...
    def test_csv_export_registers_library_row(self, tmp_path, monkeypatch):
        ...
        import json
        from backend.core.persistence import RuntimeStore
        from backend.core.agent.turn import Turn
        from backend.core.tools.types import SQLResult

        # Point exports at a tmp dir so we don't pollute the repo.
        tmp_exports = tmp_path / "exports"
        monkeypatch.setattr("backend.core.tools.handlers.create_csv_export.EXPORTS_DIR", tmp_exports)
        ...
        monkeypatch.setattr("backend.core.tools.handlers.create_csv_export.execute_export_sql", fake_execute_export_sql)
        ...
        async def _run():
            ...
            async def execute_tool(name, input_data, *, ctx=None):
                ...
                # register_export bug.
                from backend.core.tools.handlers.create_csv_export import _create_csv_export
            ...
            from backend.core.providers.types import ToolUseEvent
...
class TestSearchPlayersLimit:
    ...
    def test_negative_limit_clamped_to_1(self):
        ...
        with mock.patch("backend.core.tools.handlers.player_lookup.execute_safe_sql", return_value=dummy) as m:
...
# ---------------------------------------------------------------------------
from backend.app.processes.chat.routes import ChatResponse
...
# ---------------------------------------------------------------------------
from backend.core.tools.handlers.get_schema import _get_joins
from backend.core.tools.sandbox.schema_metadata import JOIN_EDGES
...
# ---------------------------------------------------------------------------
from backend.core.config import format_file_size
