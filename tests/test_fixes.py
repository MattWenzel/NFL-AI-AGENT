...
# ---------------------------------------------------------------------------
from core.tools.sandbox import _ensure_limit
...
class TestValidateSQLEdgeCases:
    ...
    def test_leading_block_comment_accepted(self):
        from core.tools.sandbox import validate_sql
    ...
    def test_multi_statement_rejected_by_sqlite(self):
        ...
        from core.tools.errors import SQLValidationError
...
class TestSandboxIntegration:
    ...
    def test_search_players_tool_works(self):
        ...
        _skip_if_stats_db_locked()
        from core.tools.handlers.player_lookup import _search_players
...
class TestClampLimitParam:
    ...
    def test_clamps_above_max(self):
        from core.tools.sandbox import _clamp_limit_param
...
# ---------------------------------------------------------------------------
from core.providers.clients.anthropic import AnthropicClient
from core.providers.types import Message, ToolUseEvent
...
class TestSSEDisconnectDetection:
    ...
    def test_disconnect_check_in_source(self):
        ...
        import inspect
        from app.processes.chat.routes import chat_stream
...
from core.agent.events import ToolCompletedEvent, ToolPendingEvent
...
class TestRuntimeTranscript:
    ...
    async def test_build_model_messages_preserves_tool_result_order(self, tmp_path):
        ...
        from core.agent.message_builder import build_model_messages
...
class TestCsvExportRegistration:
    ...
    def test_csv_export_registers_library_row(self, tmp_path, monkeypatch):
        ...
        from core.tools.types import SQLResult

        # Point exports at a tmp dir so we don't pollute the repo.
        tmp_exports = tmp_path / "exports"
        monkeypatch.setattr("core.tools.handlers.create_csv_export.EXPORTS_DIR", tmp_exports)
        ...
        monkeypatch.setattr("core.tools.handlers.create_csv_export.execute_export_sql", fake_execute_export_sql)
        ...
        async def _run():
            ...
            async def execute_tool(name, input_data, *, ctx=None):
                ...
                # register_export bug.
                from core.tools.handlers.create_csv_export import _create_csv_export
            ...
            from core.providers.types import ToolUseEvent
...
class TestSearchPlayersLimit:
    ...
    def test_negative_limit_clamped_to_1(self):
        ...
        with mock.patch("core.tools.handlers.player_lookup.execute_safe_sql", return_value=dummy) as m:
...
# ---------------------------------------------------------------------------
from app.processes.chat.routes import ChatResponse
...
# ---------------------------------------------------------------------------
from core.tools.handlers.get_schema import _get_joins
from core.tools.sandbox.schema_metadata import JOIN_EDGES
...
# ---------------------------------------------------------------------------
from core.config import format_file_size
