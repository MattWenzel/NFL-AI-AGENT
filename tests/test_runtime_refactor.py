...
from core.providers.types import MessageResponse, StopReason, TextEvent, ToolDefinition, ToolUseEvent, Usage
...
async def test_runtime_persists_turns_and_tool_runs(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("core.agent.runtime.execute_tool_structured", fake_execute)
    client = StubClient(
        [
            MessageResponse(
