...
- `backend/core/agent/system_prompt.py` — base system prompt template + `get_base_prompt()`.
- `backend/core/tools/guides/*.md` — seven topic-specific reference docs.
- `backend/core/tools/get_guide.py` — guide loader tool.
...
`backend/core/agent/system_prompt.py`. A single template with one variable: `{today}` plus the generated guide index. `get_base_prompt()` evaluates it at call time, so "today's date" in the prompt matches the server's clock on the day of the request.
...
Seven markdown files in `backend/core/tools/guides/` (`backend/core/tools/get_guide.py`):
...
`backend/core/tools/get_guide.py:16`. All seven files are read into a module-level dict **at import time**:
...
`backend/core/tools/get_guide.py`. Topic validation against `GUIDE_TOPICS`, then return `{"topic": ..., "content": ...}` as a JSON string. Same pattern as every other tool handler — JSON in, JSON out.

The **topic enum** is owned by `GUIDE_TOPICS` in `backend/core/tools/guide_registry.py`, and `definitions.py` derives the `get_guide` schema enum from that shared source. Adding a topic is now a one-source change instead of a manual sync across files.
...
- **Add a new guide**: (1) drop `newtopic.md` into `backend/core/tools/guides/`, (2) add `"newtopic"` to `GUIDE_TOPICS` in `backend/core/tools/guide_registry.py`, (3) add a row to `GUIDE_INDEX_ROWS` there so the system prompt stays aligned. Restart.
