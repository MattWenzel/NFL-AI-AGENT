...
`backend/domain/agent/system_prompt.py`. A single template with one variable: `{today}` plus the generated guide index. `get_base_prompt()` evaluates it at call time, so "today's date" in the prompt matches the server's clock on the day of the request.
...
- **Add a new guide**: (1) drop `newtopic.md` into `backend/domain/tools/guides/`, (2) add `"newtopic"` to `GUIDE_TOPICS` in `backend/domain/tools/guide_registry.py`, (3) add a row to `GUIDE_INDEX_ROWS` there so the system prompt stays aligned. Restart.
