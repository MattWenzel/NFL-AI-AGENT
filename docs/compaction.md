...
- `backend/domain/agent/compaction/policy.py` — trigger, retention policy, token estimation, two-phase algorithm, heuristic fallback.
- `backend/domain/agent/compaction/summarizer.py` — LLM-backed summarizer with its own input-budget trimming and timeout.
- `backend/domain/agent/compaction/token_counting.py` — `count_text_tokens` via `cl100k_base` tiktoken (lazy singleton, falls back to `len//4`).
- `backend/data/repositories/conversations/transcripts.py` + `backend/data/models.py` — persists summary turns + `compaction_summaries` rows; rebuilds wire messages with them via `message_builder.py`.
...
`summarize_for_compaction` (`summarizer.py:64`). Uses the provider's **summarizer_model** — a cheap sibling of the main model (e.g., Haiku for Anthropic, gpt-5-mini for OpenAI). Declared per provider in `backend/domain/providers/__init__.py`. See [providers.md](providers.md#summarizer-model).
...
`RuntimeStore.record_compaction` (`backend/data/repositories/conversations/transcripts.py:191`) performs three writes in a single transaction:
...
`build_model_messages` (`backend/domain/agent/message_builder.py:12`). This is where compacted context re-enters the wire protocol.
...
`_wrap_summaries_for_prompt` (`backend/domain/agent/message_builder.py`). Multiple summaries (layered compactions over a very long session) are concatenated with `---` separators, then wrapped:
...
The `Conversation Memory` section of the system prompt (`backend/domain/agent/system_prompt.py`) reinforces this — both the wrapping note and the system prompt tell the model to treat the summary as private memory.
