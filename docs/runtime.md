...
run_session(user_text)
 ...
 ├─ persistence.create_user_turn
 ├─ persistence.update_session_metadata (title on 1st turn)
 ├─ yield turn_started
...
3. **Runtime doesn't write to SQLite directly.** Every persistence concern flows through `self.persistence` or through `Turn` (which holds a reference to the same `RuntimePersistence`). The runtime's own code is almost entirely control flow.
