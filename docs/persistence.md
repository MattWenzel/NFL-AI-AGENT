...
- `backend/storage/` — `RuntimeStore` facade composed from `UsersMixin`, `SessionStoreMixin`, `TranscriptStoreMixin`, `ExportsMixin`. SQLModel-backed, async-native via aiosqlite.
...
`backend/storage/store.py`. One class with one dependency (a `Path`). Responsibilities:
...
All 9 tables are declared as SQLModel classes in `backend/storage/models.py`. Column names, defaults, indexes, and FKs are chosen to match the DB schema byte-for-byte (modulo SQLite's dynamic typing — `VARCHAR` and `TEXT` are equivalent) so existing `runtime.sqlite3` files open without migration.
...
Schema evolves via **in-house migration callables** in `backend/storage/schema_version.py`. On startup, `RuntimeStore.__init__` calls `apply_migrations(sync_engine)`:
...
All paths are idempotent. To add a new schema change: define a new `_migration_NNNN_<topic>` function that takes a `Connection`, append it to `MIGRATIONS`, and update `SQLModel` in `backend/storage/models.py` to match the post-migration shape. Keep migrations idempotent (guard with `CREATE TABLE IF NOT EXISTS` or `PRAGMA table_info` checks) so fresh DBs — where `metadata.create_all` already produces the final shape — run them as no-ops.

## Startup reconciliation

`_reconcile_interrupted_runs_sync` (`backend/storage/store.py`), called at the end of `RuntimeStore.__init__`. Two updates via the sync engine (once per process, so sync is simpler than async here):
...
`backend/storage/conversations/transcripts.py:25`. Creates a turn row with the given role/status/text. Used for:
...
There is no "streaming turn" abstraction — the turn is just a row, and `append_assistant_text` (`backend/storage/conversations/transcripts.py:70`) concatenates chunks into the `text` column in place. If the process dies mid-stream, the partial text is preserved.
...
1. **Summary turns first.** All non-compacted `role='summary'` turns become a single synthetic assistant message via `wrap_summaries_for_prompt` (`backend/storage/models.py`).
...
`backend/storage/models.py`. Single-shot snapshot returned by `get_transcript`:
...
`register_export` (`backend/storage/exports/crud.py:13`) takes the values `create_csv_export` produces and inserts an `exports` row. The owning `user_id` is resolved by looking up the session that produced the export:
