...
class RuntimePersistence:
    ...
    async def create_user_turn(self, session_id: str, text: str) -> TurnRecord:
        return await self.store.call_async(
            "create_turn", session_id, "user", text=text, status="completed"
        )

    async def open_assistant_turn(self, session_id: str) -> TurnRecord:
        return await self.store.call_async(
            "create_turn", session_id, "assistant", status="running"
        )
    ...
    async def update_session_metadata(
        ...
        *,
        ...
        session.model = model
        await self.store.call_async("update_session", session)

    async def append_assistant_text(self, session_id: str, turn_id: str, text: str) -> None:
        await self.store.call_async("append_assistant_text", session_id, turn_id, text)
    ...
    async def record_tool_call(
        ...
        *,
        ...
        tool_run = await self.store.call_async(
            "create_tool_run",
            session_id,
            turn_id,
            tool_name,
    ...
    async def update_turn(self, turn_id: str, **changes) -> TurnRecord:
        return await self.store.call_async("update_turn", turn_id, **changes)

    async def get_turn(self, turn_id: str) -> TurnRecord | None:
        return await self.store.call_async("get_turn", turn_id)

    async def get_tool_run(self, tool_run_id: str) -> ToolRunRecord | None:
        return await self.store.call_async("get_tool_run", tool_run_id)

    async def build_model_messages(self, session_id: str):
        transcript = await self.store.call_async("get_transcript", session_id)
    ...
    async def begin_tool_execution(self, session_id: str, turn_id: str, tool_run_id: str, tool_name: str) -> None:
        await self.store.call_async("update_tool_run", tool_run_id, status="running")
        await self.store.call_async(
            "add_part",
            session_id,
            turn_id,
